...
import { Clock, SearchBox, SystemMonitor, StickyNotes } from '@/components/deskModule'
...
// 背景图预加载
const bgPreloaded = ref(false)
function preloadBackground() {
  const src = panelState.panelConfig.backgroundImageSrc
  if (!src) {
    bgPreloaded.value = true
    return
  }
  const img = new Image()
  img.onload = () => { bgPreloaded.value = true }
  img.onerror = () => { bgPreloaded.value = true }
  img.src = src
}
...
// 悬浮按钮拖拽
const fixedElementRef = ref<HTMLElement | null>(null)
const fixedElPos = ref({ x: 20, y: 50 })
const isDragging = ref(false)
const dragStart = ref({ x: 0, y: 0, elX: 20, elY: 50 })

// 保存菜单位置到 localStorage
function saveMenuPos() {
  localStorage.setItem('sun-panel-menu-pos', JSON.stringify(fixedElPos.value))
}
function loadMenuPos() {
  try {
    const saved = localStorage.getItem('sun-panel-menu-pos')
    if (saved) {
      const pos = JSON.parse(saved)
      if (pos.x !== undefined && pos.y !== undefined) {
        fixedElPos.value = { x: pos.x, y: pos.y }
        dragStart.value = { x: 0, y: 0, elX: pos.x, elY: pos.y }
      }
    }
  } catch { /* ignore */ }
}
...
function onFixedElDragStart(e: MouseEvent | TouchEvent) {
  const target = e.target as HTMLElement
  // 如果点击的是按钮组件及其内部元素，不拖拽
  if (target.closest('.n-button') || target.closest('.not-drag-handle')) return
  isDragging.value = true
  const clientX = 'touches' in e ? e.touches[0].clientX : e.clientX
  const clientY = 'touches' in e ? e.touches[0].clientY : e.clientY
  dragStart.value = { x: clientX, y: clientY, elX: fixedElPos.value.x, elY: fixedElPos.value.y }
  document.addEventListener('mousemove', onFixedElDragMove)
  document.addEventListener('mouseup', onFixedElDragEnd)
  document.addEventListener('touchmove', onFixedElDragMove)
  document.addEventListener('touchend', onFixedElDragEnd)
  e.preventDefault()
}

function onFixedElDragMove(e: MouseEvent | TouchEvent) {
  if (!isDragging.value) return
  const clientX = 'touches' in e ? e.touches[0].clientX : e.clientX
  const clientY = 'touches' in e ? e.touches[0].clientY : e.clientY
  const dx = dragStart.value.x - clientX
  const dy = dragStart.value.y - clientY
  fixedElPos.value = {
    x: Math.max(0, Math.min(window.innerWidth - 60, dragStart.value.elX + dx)),
    y: Math.max(0, Math.min(window.innerHeight - 200, dragStart.value.elY + dy)),
  }
}

function onFixedElDragEnd() {
  isDragging.value = false
  document.removeEventListener('mousemove', onFixedElDragMove)
  document.removeEventListener('mouseup', onFixedElDragEnd)
  document.removeEventListener('touchmove', onFixedElDragMove)
  document.removeEventListener('touchend', onFixedElDragEnd)
  saveMenuPos()
}
...
async function loadSystemLogo() {
  try {
    const res = await getLogoConfig<any>()
    if (res.code === 0 && res.data) {
      systemLogoUrl.value = res.data.imageUrl || ''
      systemLogoSize.value = res.data.size || 80
    }
  } catch { /* ignore */ }
}
...
async function autoDetectLan() {
  if (!panelState.panelConfig.autoDetectLan)
    return

  const detectUrl = panelState.panelConfig.lanDetectUrl || window.location.origin

  // 如果当前已经是 LAN IP 访问，直接切换
  const hostname = window.location.hostname
  const isLanIp = /^(10\.|172\.(1[6-9]|2\d|3[01])\.|192\.168\.)/.test(hostname)
  if (isLanIp) {
    if (panelState.networkMode !== PanelStateNetworkModeEnum.lan) {
      panelState.setNetworkMode(PanelStateNetworkModeEnum.lan)
      ms.success(t('panelHome.autoDetectLanSuccess'))
    }
    return
  }

  // 如果已是 WAN 模式且当前不在 LAN IP，尝试探测 LAN 地址
  if (panelState.networkMode === PanelStateNetworkModeEnum.wan) {
    try {
      const controller = new AbortController()
      const timeoutId = setTimeout(() => controller.abort(), 2000)
      await fetch(detectUrl, { mode: 'no-cors', signal: controller.signal })
...
<template>
  <div class="w-full h-full sun-main">
    <div
      class="cover wallpaper" :class="{ 'bg-loaded': bgPreloaded }" :style="{
          ...
          <div class="flex mx-[auto] items-center justify-center text-white">
            <div class="logo flex items-center gap-3">
              <img
                v-if="systemLogoUrl"
                :src="systemLogoUrl"
                :style="{ width: `${systemLogoSize}px`, height: `${systemLogoSize}px` }"
    ...
    <div
      ref="fixedElementRef"
      class="fixed-element shadow-[0_0_10px_2px_rgba(0,0,0,0.2)]"
      :style="{ right: fixedElPos.x + 'px', bottom: fixedElPos.y + 'px' }"
    >
      <div
        class="fixed-element-drag-handle"
        :title="'拖拽移动'"
        @mousedown="onFixedElDragStart"
        @touchstart="onFixedElDragStart"
      >
        <SvgIcon class="drag-icon" icon="mdi:drag-vertical-variant" />
      </div>
      <NButtonGroup vertical size="small" class="not-drag-handle">
        <!-- 网络模式切换按钮组 -->
        <NButton
        ...
        >
          <template #icon>
            <SvgIcon class="text-white" icon="material-symbols:lan-outline-rounded" :style="{ fontSize: '20px' }" />
          </template>
        </NButton>
        ...
        >
          <template #icon>
            <SvgIcon class="text-white" icon="mdi:wan" :style="{ fontSize: '20px' }" />
          </template>
        </NButton>

        <NButton v-if="authStore.visitMode === VisitMode.VISIT_MODE_LOGIN" color="#2a2a2a6b" @click="settingModalShow = !settingModalShow">
          <template #icon>
            <SvgIcon class="text-white" icon="majesticons-applications" :style="{ fontSize: '20px' }" />
          </template>
        </NButton>
        ...
        <NButton v-if="authStore.visitMode === VisitMode.VISIT_MODE_PUBLIC" color="#2a2a2a6b" :title="$t('panelHome.goToLogin')" @click="router.push('/login')">
          <template #icon>
            <SvgIcon class="text-white" icon="material-symbols:account-circle" :style="{ fontSize: '20px' }" />
          </template>
        </NButton>
      </NButtonGroup>
...
.fixed-element-drag-handle:hover {
  opacity: 1;
}

.fixed-element-drag-handle:active {
  cursor: grabbing;
}
