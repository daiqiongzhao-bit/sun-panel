  ...
  <div class="bg-slate-200 dark:bg-zinc-900 rounded-[10px] p-[8px] overflow-auto">
      ...
      >
        <NUploadDragger style="width: 100%;">
          <div
            ...
            :style="{ backgroundImage: `url(${panelState.panelConfig.backgroundImageSrc})`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat', backgroundColor: '#e2e8f0' }"
          >
    ...
    <NCard style="border-radius:10px" class="mt-[10px]" size="small">
      ...
      <NGrid cols="2">
        ...
        <NGridItem span="12 400:12">
          <div class="flex items-center mt-[5px]">
            <span class="mr-[10px]">{{ $t('apps.baseSettings.autoDetectLan') }}</span>
            <NSwitch v-model:value="panelState.panelConfig.autoDetectLan" />
          </div>
          <div v-if="panelState.panelConfig.autoDetectLan" class="text-xs text-slate-400 mt-1">
            {{ $t('apps.baseSettings.autoDetectLanDesc') }}
          </div>
        </NGridItem>

        <NGridItem v-if="panelState.panelConfig.autoDetectLan" span="12 400:12">
          <div class="flex items-center mt-[5px]">
            <span class="mr-[10px]">{{ $t('apps.baseSettings.lanDetectUrl') }}</span>
            <NInput
              v-model:value="panelState.panelConfig.lanDetectUrl"
              size="small"
              :placeholder="$t('apps.baseSettings.lanDetectUrlPlaceholder')"
              :style="{ width: '280px' }"
            />
          </div>
        </NGridItem>
