...
function parseBookmarkHtml(html: string): BookmarkNode[] {
  ...
  function parseDT(dl: Element): BookmarkNode[] {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (item.tagName === 'DT') {
        ...
        if (anchor) {
          ...
          const subDT = item.querySelector('dl')
          if (h3 && subDT) {
            nodes.push({
              title: h3.textContent || '',
              children: parseDT(subDT),
            })
          }
        }
      }
...
function convertBookmarksToIconGroups(bookmarks: BookmarkNode[]): IconGroup[] {
  ...
  function processNodes(nodes: BookmarkNode[], parentGroup?: IconGroup) {
    ...
    if (leafItems.length > 0) {
      if (parentGroup) {
        parentGroup.children = leafItems
      }
      groups.push({
        title: parentGroup?.title || '书签导入',
        sort: groups.length,
        children: leafItems,
      })
    }
  ...
  processNodes(bookmarks)
  return groups
}

function handleBookmarkFile(options: { file: UploadFileInfo; fileList: Array<UploadFileInfo> }) {
  ...
  <div class="pt-2">
    ...
    <div class="flex justify-center mt-4">
      <div class="text-center">
        <p class="text-sm text-gray-500 mb-2">{{ $t('apps.exportImport.transmuteStandard') }}</p>
        <NUpload
          accept=".html,.htm"
          :default-upload="false"
          :show-file-list="false"
          @change="handleBookmarkFile"
        >
          <NButton size="small" ghost type="info">
            <template #icon>
              <SvgIcon icon="mdi:bookmark-multiple-outline" />
            </template>
            选择书签HTML文件
          </NButton>
        </NUpload>
        <p class="text-xs text-gray-400 mt-1">支持Chrome/Firefox/Edge浏览器导出的书签文件</p>
      </div>
    </div>
