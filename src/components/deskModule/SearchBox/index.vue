...
import { NAvatar, NCheckbox, NTag } from 'naive-ui'
import { SvgIcon } from '@/components/common'
import { useModuleConfig } from '@/store/modules'
import { useAuthStore } from '@/store'
import { VisitMode } from '@/enums/auth'
import { localSearch } from '@/api/panel/search'
...
// 本地搜索
const localSearchResults = ref<any[]>([])
const localSearchShow = ref(false)
...
function handleLocalSearch() {
  ...
  searchTimer = setTimeout(() => {
    ...
    localSearch<Common.ListResponse<any[]>>({ keyword, limit: 5 }).then(({ code, data }) => {
      if (code === 0) {
        localSearchResults.value = data.list || []
        localSearchShow.value = localSearchResults.value.length > 0
      }
    }).finally(() => {
...
function handleLocalResultClick(item: any) {
  localSearchShow.value = false
  if (item.type === 'icon') {
    // 根据网络模式选择 URL
  ...
  else if (item.type === 'pastebin') {
    navigator.clipboard.writeText(item.code).then(() => {
      // 访问码已复制
    })
  }
  // group type: just close dropdown, no action
}

// Watch searchTerm for local search
watch(searchTerm, () => {
  handleLocalSearch()
})
...
<template>
  <div class="search-box w-full" @keydown.enter="handleSearchClick" @keydown.esc="handleClearSearchTerm">
    ...
    <!-- 本地资产搜索下拉 -->
    <div
      v-if="localSearchShow && localSearchResults.length > 0"
      class="w-full mt-[10px] rounded-xl p-[10px] local-search-dropdown"
      :style="{ background, color: textColor }"
    >
      <div class="text-xs opacity-70 mb-2 px-2">{{ $t('deskModule.searchBox.localSearch') }}</div>
      <div
        v-for="(item, index) in localSearchResults"
        :key="index"
        class="flex items-center px-3 py-2 rounded-lg cursor-pointer hover:bg-white/10 transition-colors"
        @mousedown.prevent="handleLocalResultClick(item)"
      >
        <NTag :type="item.type === 'icon' ? 'info' : item.type === 'pastebin' ? 'warning' : 'success'" size="tiny" class="mr-2">
          {{ item.type === 'icon' ? '🔗' : item.type === 'pastebin' ? '📋' : '📁' }}
        </NTag>
        <div class="flex-1">
          <div class="text-sm font-medium">{{ item.title }}</div>
          <div v-if="item.desc" class="text-xs opacity-60 truncate">{{ item.desc }}</div>
        </div>
        <span v-if="item.type === 'pastebin'" class="text-xs opacity-50">{{ item.code }}</span>
      </div>
    </div>

    <!-- 搜索引擎选择 -->
    <div class="w-full mt-[10px] rounded-xl p-[10px]" :style="{ background }">
